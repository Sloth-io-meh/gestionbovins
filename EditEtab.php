...
if (isset($_POST["submit"])){
  ...
  global $link;
  $editing = $_GET['Edit'];
  $req = "UPDATE etables set nom='$nomA' where id_etab='$editing' ";
...
$searching= $_GET['Edit'];
global $link;
$req="Select * From etables where id_Etab ='$searching' ";
$exec=mysqli_query($link, $req);
while ($datarows = mysqli_fetch_array($exec)) {
$nomupdate =$datarows['nom'];
}
?>
    <form class="" action="EditEtab.php?Edit=<?php echo $searching ; ?>" method="post">
      <fieldset>

        <div class="form-group col-md-6">
          <label for="type">Nom Etable:</label>
          <input required  type="text" class="form-control" name="nometab" value=" <?php echo $nomupdate ?> ">
        </div>
        <input required  class="btn btn-primary btn-block" type="submit" name="submit" value="Modifer">
      </fieldset>
    </form>
  </div>
          </div>
