...
if (isset($_POST["submit"])){
  ...
  $dateX = $_POST["dateX"];
  global $link;
  $req = "INSERT INTO stock(libelle_st,description_s,quantite_s,quantiteAct,prix_s,dateachat,dateexp_s)VALUES('$nom','$desc','$q','$q','$p','$dateA','$dateX')";
...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
      ...
      <div class="row">
        <div class="col-sm-2 ">
          ...
          <li> <div class="">
            ...
            <li class="active"><a href="ajoutStock.php"><span class="glyphicon glyphicon-th"></span>&nbsp;Stock Alimentation</a></li>
        ...
        <div class="col-sm-10">
          <h1>Gestion De Stock Alimentation</h1>
          <div class="">
          <?php echo Message();
                echo SuccessMessage();
          ?>
  ...
  <div class="panel-body">
    <form class="" action="ajoutStock.php" method="post">
      <fieldset>

        <div class="form-group col-md-6">
          <label for="type">Nom Produit:</label>
          <input required  type="text" class="form-control" name="nomMed" value="">
        </div>
        <div class="form-group col-md-6">
          <label for="type">Description Produit:</label>
          <textarea class="form-control" name="describeMed" required></textarea>
        </div>
        <div class="form-group col-md-6">
          <label for="type">Quantité:</label>
          <input required  type="number" class="form-control" name="quantite" value="">
        </div>
        <div class="form-group col-md-6">
          <label for="type">prix:</label>
          <input required  type="text" class="form-control" name="prix" value="">
        </div>
        <div class="form-group col-md-6">
          <label for="type">Date d'achat:</label>
          <input required  type="date" class="form-control" name="dateA" value="">
        </div>
        <div class="form-group col-md-6">
          <label for="type">Date d'expiration:</label>
          <input required  type="date" class="form-control" name="dateX" value="">
        </div>
        <input required  class="btn btn-primary btn-block" type="submit" name="submit" value="Ajouter">
          ...
          <div class="panel panel-primary">
            <div class="panel-heading">Les Produits Availables/ajouté</div>
<div class="panel-body">
  ...
  <?php
            global $link;
            $view = "SELECT * FROM stock";
            $exec=mysqli_query($link,$view);
            $sr = 0;
            while ($datarows=mysqli_fetch_array($exec)) {
              $Id = $datarows["id_stock"];
              $libelle=$datarows["libelle_st"];
              $desc = $datarows["description_s"];
              $q = $datarows["quantite_s"];
              $q2 = $datarows["quantiteAct"];
              $p = $datarows["prix_s"];
              $dateA = $datarows["dateachat"];
              $dateX = $datarows["dateexp_s"];
              $sr++;
              ?>
              <tr>
              <td><?php echo  $sr;?></td>
              ...
              <td><?php echo $desc; ?></td>
              <td><?php echo $q." Kg"; ?></td>
              <td><?php echo $q2.' Kg'; ?></td>
              <td><?php echo " ".$p." DH"; ?></td>
              <td><?php echo $dateA; ?></td>
              <td><?php echo $dateX; ?></td>
              <td><a href="EditStock.php?Edit=<?php echo $Id; ?>"><span class="btn btn-warning">modifier &nbsp;&nbsp;</span></a>
               <a href="DeleteStock.php?id=<?php echo $Id; ?>"> <span class="btn btn-danger">supprimer</span></a></td>
   </tr>
 <?php } ?>
  </table>
</div>
